  ...
  return (
    <div className="flex w-full flex-col border border-1 rounded-lg overflow-hidden">
      <Tabs
        ...
        className="gap-9 md:gap-0 overflow-hidden border-0 "
      >
        <TabsList className="w-full flex flex-wrap text-[12px] sm:text-[16px] font-semibold gap-0 sm:gap-0 bg-white p-0">
          {tabs.map((item, index) => (
            <TabsTrigger
              value={item.value}
              key={index}
              className=" px-5 data-[state=active]:bg-[#F5F5F5] data-[state=active]:text-primary data-[state=active]:rounded-none "
            >
              {item.label}
            </TabsTrigger>
          ))}
        ...
        {tabs.map((tab, index) => (
          <TabsContent value={tab.value} key={index}>
            <Card className="bg-[#F5F5F5] p-[30px] rounded-none border-none">
              {tab.component}
            </Card>
          </TabsContent>
        ))}
